...
{
  ProductBloc():super(ProductLoadedState(listProduct)){
    on<SearchProductsEvent>((event, emit) {
      List<Product> listFound=listProduct.where((element) => element.name.toLowerCase().contains(event.name.toLowerCase())).toList();
      return emit(ProductFoundState(listFound));
    });
    on<SameProductInitialEvent>((event, emit) {
      List<Product> sameProducts=listProduct.where((element) => element.brand==event.product.brand&&element!=event.product).toList();
      return emit(ProductSameState(listProduct, sameProducts));
    });
  }
